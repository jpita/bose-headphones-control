...
struct NativeHeadphoneState {
    ...
    var model = "Bose Headphones Control Swift"
    var battery: Int?
    var firmware = "—"
    var mode = "—"
    var modeIndex: Int?
...
final class NativeBackend: ObservableObject {
    ...
    private let bluetooth = BoseRFCOMMClient()
    private let logger = Logger(subsystem: "com.jpita.bose-headphones-control.swift", category: "Backend")
    ...
    func refresh() { Task { await refreshState() } }

    func reconnect() {
        Task {
            isBusy = true
            message = "Reconnecting over native Bluetooth…"
            bluetooth.disconnect()
            state.connected = false
            await connectAndLoad()
            isBusy = false
    ...
    func setMode(_ mode: String, announce: Bool) {
        action("Mode: \(mode)") {
            guard let profile = self.state.profiles.first(where: { $0.name.caseInsensitiveCompare(mode) == .orderedSame }) else {
                throw BMAPError.unsupported("Unknown listening mode: \(mode)")
            }
            _ = try await self.request(31, 3, .start, Data([UInt8(profile.id), announce ? 1 : 0]))
        }
    }
    ...
    private func connectAndLoad() async {
        do {
            logger.info("Starting native Swift backend")
            print("[Backend] Starting native Swift backend")
            message = "Looking for paired Bose headphones…"
            ...
            message = "Connected natively — no Python service."
            logger.info("Native Swift backend connected and loaded state")
            print("[Backend] Loaded battery=\(state.battery.map(String.init) ?? "unknown") mode=\(state.mode) profiles=\(state.profiles.count) features=\(state.features.sorted().joined(separator: ","))")
        } catch {
            state.connected = false
            state.name = "Headphones unavailable"
            message = error.localizedDescription
            logger.error("Native backend failed: \(error.localizedDescription, privacy: .public)")
            print("[Backend] Native backend failed: \(error.localizedDescription)")
        }
    }
    ...
    private func refreshState(silent: Bool = false) async {
        do {
            ...
            try await loadState()
            if !silent { message = "Connected natively — no Python service." }
        } catch {
            state.connected = false
    ...
    private func loadState() async throws {
        ...
        if !configs.isEmpty { next.features.formUnion(["current_mode", "mode_config"]) }
